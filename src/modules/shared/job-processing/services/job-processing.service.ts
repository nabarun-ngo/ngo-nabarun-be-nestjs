...
import { Job, JobOptions, FlowJob, FlowOptions } from '../dto/job.dto';
...
export class JobProcessingService {
  ...
  constructor(
    ...
    @InjectFlowProducer(config.jobProcessing.queueName + '-flow-producer')
  ...
  async addJob<T>(
    ...
    try {
      ...
      // 4. Ensure jobId is not a pure integer
      if (jobOptions.jobId) {
        jobOptions.jobId = this.ensureStringId(jobOptions.jobId);
      }
  ...
  async addJobToQueue<T>(
    ...
    try {
      ...
      if (mergedOptions.jobId) {
        mergedOptions.jobId = this.ensureStringId(mergedOptions.jobId);
      }
  ...
  /**
   * Add a job flow (Parent-Child relationship)
   * This is the recommended way to handle jobs that depend on other jobs.
   * The parent job will not start until all children have completed.
   */
  async addJobFlow(
    flow: FlowJob,
    options?: FlowOptions,
    ...
    try {
      this.logger.log(`Adding job flow with parent: ${flow.name}`);

      // Apply TTL recursively to all jobs in the flow
      const flowWithTTL = this.setFlowTTL(flow);

      const flowNode = await this.flowProducer.add(flowWithTTL, options);

      this.logger.log(`Job flow added successfully. Parent ID: ${flowNode.job.id}`);
      return flowNode.job as Job;
    } catch (error) {
      this.logger.error(`Failed to add job flow: ${flow.name}`, error);
      throw error;
    }
  }

  /**
   * Add children to an ALREADY RUNNING job.
   * This will turn the current job into a "Waiting Parent".
   * Note: The parent job must return after calling this to enter the waiting state.
   */
  async addChildrenToJob(
    parentJob: Job,
    children: FlowJob[],
  ): Promise<void> {
    try {
      this.logger.log(`Adding ${children.length} children to parent job: ${parentJob.id}`);

      // Apply TTL recursively to children
      const childrenWithTTL = children.map(child => this.setFlowTTL(child));

      // 4. Ensure current job ID is used as the parent in the flow
      // If parentJob.id was auto-generated by BullMQ, it might be a numeric string
      // but FlowProducer.add considers it a 'custom ID' when passed in opts
      const parentJobId = this.ensureStringId(parentJob.id!);

      // Use the flow producer to add children to the existing job
      await this.flowProducer.add({
        name: parentJob.name,
        queueName: config.jobProcessing.queueName,
        data: parentJob.data,
        opts: { ...parentJob.opts, jobId: parentJobId }, // Target the existing job ID
        children: childrenWithTTL,
      });

      this.logger.log(`Children attached to job ${parentJob.id}. Job will enter waiting-children state upon completion of current phase.`);
    } catch (error) {
      this.logger.error(`Failed to add children to job ${parentJob.id}`, error);
      throw error;
    }
  }
  ...
  /**
   * Set TTL for flows based on configuration recursively
   */
  private setFlowTTL(flow: FlowJob): FlowJob {
    const jobOptions = this.setJobTTL(flow.opts as JobOptions);
    ...
    return {
      ...flow,
      opts: jobOptions,
      children: flow.children?.map(child => this.setFlowTTL(child))
    };
  }

  /**
   * Ensures that a custom job ID is not a pure integer, which BullMQ 5 rejects.
   */
  private ensureStringId(id: string | number): string {
    const stringId = String(id);
    // If it's purely numeric, prefix it
    if (/^\d+$/.test(stringId)) {
      return `job-${stringId}`;
    }
    return stringId;
